...
| 脚本 | 当前职责 | 权威状态 | 拆分结论 |
|---|---|---|---|
| `scripts/gameplay/DrinkWorkstation.cs` | 双手、拿放、砧板、材料转移、工序选择/结算、饮品、卫生、水壶、评价、反馈 | 大部分当日制作状态 | 待拆 P1（最高优先级） |
| `scripts/gameplay/DrinkWorkstation.Persistence.cs` | 工作台状态与 `WorkstationSnapshot` 往返 | 不新增状态 | 已从主文件拆出；以后移入 coordinator/mapper |
...
| 脚本/目录 | 职责 | 结论 |
|---|---|---|
| `tests/DomainTests.cs` | 纯 C# 日流程、液体、评价、工序、动作定义、目录校验、存档 schema | 保留；当前 16 项 |
