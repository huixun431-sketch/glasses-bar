...
- `OpeningMenuController` 在最终视觉稿的无字背景/独立标题上管理 Godot 实时菜单文字、透明点击区、输入模式和独立选择器；“继续游戏”因尚无存档而明确禁用。鼠标未悬停时选择器隐藏，键盘/手柄导航时显示。`PauseMenuController` 以 `ProcessMode.Always` 在暂停状态接收输入。
- `MyopiaProgression` 以纯领域函数计算 30 天近视值；`MyopiaEffectController` 在 `DayChanged` 时应用规则值，开发控制台可临时覆盖。
- 前台、后吧台、左右回转台均有玩家碰撞，形成 `1.80 m` 两人工作通道；两个 `0.70 m` 玩家碰撞直径并排后仍有 `0.40 m` 总余量。前吧台是 10 个深抽屉，后柜使用 12 扇成对窄外开门；`CabinetInteractable` 限制抽屉行程并用单开互锁，任一储物打开后最窄处仍约 `1.28 m`。冰桶随指定抽屉移动，其余柜体当前不保存正式玩法物品。
...
- 自动化必须覆盖双手容量、实体移动、防重叠、复数砧板工具、冲突、单一载料、错误工具/材料、比例成功/失败、手动清废、每日洗手、双头量酒器、固定水壶、重复工序恢复、柜体通行、无戴镜制作、交付与跨天重置。
