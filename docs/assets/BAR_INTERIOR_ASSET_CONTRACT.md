...
| 前吧 | `bar_counter` | 连续 C 形基座/柜体/台面、抽屉、水槽/弃物/员工门/手册架可见件 | 抽屉、门、手册和站点玩法；稳定 ID；碰撞；储物状态 |
| 后吧 | `bar_backbar` | 后吧台、五湾两层瓶架、五组推拉下柜和十扇上柜可见件 | 推拉/铰链运动、互锁、柜内宿主、碰撞和每日复位 |
| 家具 | `bar_furniture` | 六吧椅、三圆桌、十二椅 | 通路诊断、可选碰撞和布局权威坐标 |
| 灯具 | `bar_lighting` | 三前吧吊灯、三客桌吊灯、两后吧线性灯外壳、四壁灯外壳 | 所有真实 Light3D、能量、颜色、阴影和世界切换 |
...
- 固定功能节点：`east_sink`、`sink_base`、`waste_bin`、`employee_gate`、`manual_shelf`、`bar_structural_base`、`bar_carcass_monolith`、`bar_worktop_monolith`。
- 前、侧、后吧的结构基座、柜体和工作台分别保持单一连续 C 形网格；仅在东侧员工门处切分。内侧工作边缘使用 45° 倒角，外侧不得生成倒角凸块。
- 客侧台面必须比未移动的立板向 `+Z` 外挑 `0.30 m`。抽屉所在台下保持空心柜架，不得恢复与抽屉包络重叠的实心棱柱。
- `east_sink` 下保留无抽屉的实心 `sink_base`，不得导出可见管线。水槽必须有实际贯通的空心槽体；中心向下射线首先命中槽底，不得被台面、竖直槽框或异物截断。龙头位于东侧槽沿，只用短下倾出水嘴伸入槽内。
- `manual_shelf` 位于西侧；相对上一版横跨转角的摆放顺时针旋转 `90°`，世界包络约为东西 `0.28 m` × 南北 `0.62 m`，长边贴合西侧吧台纵深且不得侵入转角或玩家通道。东侧前吧下方不得恢复已取消的柜体。
...
- 实际交付包含连续编号的六吧椅、三圆桌、十二椅，各自保持独立对象与可识别变换；每把客座椅的前向必须指向所属桌心。

### `bar_lighting`

- 必需节点：`pendant_1`、`pendant_3`、`lounge_pendant_1/2/3`、`rear_linear_1/2`、`east_sconce_1/2`、`west_sconce_1/2`。
- GLB 中不得包含 Light 节点；这里只导出可见外壳。顾客区无可见灯具的填光不进入本模块。
- 所有灯罩使用稳定材质名 `frosted_translucent_shade`，以半透明、透射和高粗糙度标注磨砂扩散响应。三盏客桌吊灯各自居中于桌面；Godot 包装提供足以照亮台面的向下 SpotLight3D。
