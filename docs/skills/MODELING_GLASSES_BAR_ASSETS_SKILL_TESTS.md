...
- Integration exposed that the repository legitimately tracks Godot-generated `.glb.import` files for approved formal assets. A regression test now proves those existing imports do not block a new batch, while tracked `artifacts/`, review PNGs, and `.blend` files remain rejected. Manual `.glb.import` edits remain a Git-diff review rule because tracked state alone cannot distinguish them from Godot-generated metadata.
