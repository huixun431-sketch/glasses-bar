...
- 前吧台顶高为 `1.18 m`，后吧台顶高为 `1.11 m`，玩家视点同步抬到约 `1.76 m`；根据最新反馈，内侧净宽由 `1.24 m` 拉开到 `1.80 m`，可容纳两个 `0.70 m` 玩家碰撞直径并排并保留 `0.40 m` 总余量。
- 后吧台和酒架合并，全部初始工具移到前吧台；吧台外顾客区扩展并加入更多卡座、桌椅和走动空间。
- 前吧台使用 5 组双层深抽屉（共 10 个），不再放柜门；后吧台 6 个柜体使用 12 扇成对窄门并向操作通道一侧外开。水槽左侧另有上下两层窄抽屉，位置不再卡入墙体。
- 深抽屉保留 `0.58–0.76 m` 箱体深度，但只拉出可用前段；任一柜门/抽屉打开后最窄处仍约 `1.28 m`，足够一人舒适通过。柜门/抽屉互锁继续保留，避免前后储物同时伸入通道。
...
- Windows 包已重导出并隐藏启动，Debug/Release 退出码均为 `0`：
...
- 工作通道最终按用户追加反馈改为 `1.80 m` 两人宽度；柜门必须向外开，任一储物打开后仍需保留至少一人通行。后柜窄双门、抽屉有限行程和单开互锁继续作为防困措施。
...
1. 使用新版 Release 至少完整游玩两天，重点检查：鼠标/键盘菜单切换、吧台与视点高度、任一柜门/抽屉打开后的通行、冰桶取用、每日洗手、三种量酒器换端、空水壶诊断、重复萃取/过滤补救。
2. 记录 `1.80 m` 两人通道、柜体互锁、抽屉拉出长度、量酒器操作次数和 `4%` 未洗手惩罚在实际手感上是否合适。
...
> 请先阅读 `progress.md` 和 `docs/CONTEXT_HANDOFF.md`，使用项目 Skill `develop-glasses-bar-godot` 接手。先实玩新版 Release 两天，重点检查拆分主菜单的鼠标/键盘高亮、`1.80 m` 两人通道与柜体互锁、冰桶抽屉、每日洗手、三种双头量酒器、空水壶诊断和重复工序补救，再按实际手感继续迭代。
