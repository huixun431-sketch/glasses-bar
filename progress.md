...
1. 写入并自检新的测试优先实施计划，先观察新合同测试在旧基线失败。
2. 重建布局、连续台体、五湾收纳、柜内物品宿主／门禁、五湾空瓶架和运行时包络校验。
3. 完成全量回归、Debug／Release 构建和 16 张 1920×1080 Forward+ 截图，人工检查后交由用户批准。
4. 用户批准新灰盒前，不开始正式环境 GLB 合同或 Blender 几何。
