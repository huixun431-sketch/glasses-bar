...
- 前吧台顶高为 `1.18 m`，后吧台顶高为 `1.11 m`，玩家视点同步抬到约 `1.76 m`；内侧净宽固定为 `1.24 m` 的单人操作通道。
...
- 深抽屉保留 `0.58–0.76 m` 箱体深度，但只拉出可用前段；后柜窄门全开时也至少保留一个玩家碰撞直径（`0.70 m`）的通行带。柜门/抽屉互锁，同一时间只让一处储物正面伸入单人通道，避免前后同时夹死玩家。
...
- 工作通道保持单人宽度。柜门必须向外开，但储物开启不能困住玩家；因此后柜采用窄双门，深抽屉只拉出前段，并以单开互锁维持通行。
...
2. 记录 `1.24 m` 单人通道、柜体互锁、抽屉拉出长度、量酒器操作次数和 `4%` 未洗手惩罚在实际手感上是否合适。
...
> 请先阅读 `progress.md` 和 `docs/CONTEXT_HANDOFF.md`，使用项目 Skill `develop-glasses-bar-godot` 接手。先实玩新版 Release 两天，重点检查拆分主菜单的鼠标/键盘高亮、单人通道与柜体互锁、冰桶抽屉、每日洗手、三种双头量酒器、空水壶诊断和重复工序补救，再按实际手感继续迭代。
